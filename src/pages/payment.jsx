...
const DateTimePicker = dynamic(() => import("../components/DatePciker/DatePciker"), { ssr: false });
...
const Payment = observer(() => {
  ...
  const uploadCheck = async () => {
    ...
    const date = `${time.getDate()}.${time.getMonth() + 1}.${time.getFullYear()} ${time.getHours()}:${
    ...
    formData.append(
      "caption",
      `Телефон: ${Store.form.tel} \nЗаголовок: ${Store.form.title} \nВремя: ${
        !defaultTime ? "30 минут" : date
  ...
  return (
    <Layout>
      <div className="w-full flex justify-center ">{/* <Image className="mt-[52px]" src={logo}/> */}</div>
      ...
      >
        ...
        {defaultTime && <DateTimePicker className="clock_dis" time={time} openClockOnFocus={false} setTime={setTime} />}
        {!defaultTime && (
          <div>
        ...
        {!defaultTime ? (
          <p onClick={() => setDefaultTime(true)} className="text-center text-xs mt-3 text-gray-400">
            Выбрать время
          </p>
        ) : (
          <p onClick={() => setDefaultTime(false)} className="text-center text-xs mt-3 text-gray-400">
            Срочный запуск
          </p>
        )}
            ...
            {payments.map((item) => {
              return (
                <li key={item.name} className="flex items-center gap-2">
                  ...
                  <p className="text-sm">{item.transaction}</p>
                </li>
              );
            })}
          </ul>
        ...
        <label className="flex justify-center my-4">
          <div
            className="rounded-lg mt-2  relative z-10 overflow-hidden border-[.5px] flex justify-center items-center flex-col border-stroke w-[200px] px-[13px] h-[77px]"
            style={{
              background:
          ...
          >
            ...
            {preview && (
              <div className="text-white text-center">
                <Image src={successIcon} width={40} alt="check" /> Чек
              </div>
            )}
          </div>
          <input
      ...
      <div className="flex justify-center pb-12 pt-5">
        <button
          ...
          className="w-[314px]  h-[50px] rounded-[5px] text-white"
        >
          Дальше
        </button>
      </div>
